...
async def _memory_heartbeat(interval_s: float = 30.0):
    ...
    On the OLD (gather) code, 'active_connections' climbs with every connect and
    never falls after disconnect — that monotonic climb (matched by rising rss)
    is the leak. On the fixed code it returns to baseline after each disconnect.
    """
    while True:
